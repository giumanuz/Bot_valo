...
class ConversationTests(unittest.TestCase):
    ...
    def test_Foto_ifTriggerWordSent_ShouldSendPhoto(self):
        res = self._send_fake_message_to(Foto, "mazza")
        self.assertEqual(1, len(res))
    ...
    def test_Foto_ifTextContainsTrigger_ShouldSendPhoto(self):
        res = self._send_fake_message_to(Foto, "test mazza test")
        self.assertEqual(1, len(res))
        self.assertHasValidPhoto(res)

    def test_Risposte_ifTextContainsNonExplicitTrigger_ShouldNotSendPhoto(self):
        res = self._send_fake_message_to(Foto, "testmazzatest")
        self.assertEqual(0, len(res))
    ...
    def test_Risposte_ifTextContainsExplicitTrigger_ShouldReply(self):
        res = self._send_fake_message_to(Risposte, "this is a test")
        self.assertEqual(1, len(res))
